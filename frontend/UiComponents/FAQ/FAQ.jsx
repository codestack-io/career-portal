...
export default function FAQ({ faqs = [] }) {
  ...
  // Extract array safely if backend returns DRF pagination
  const faqList = Array.isArray(faqs) ? faqs : faqs?.results || [];

  // Toggle between 5 items and full list
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        <div className="mb-20 text-center">
          ...
          <p className="mx-auto mt-6 max-w-3xl text-lg leading-8 text-slate-600">
            Find answers about admissions, scholarships, visas, universities and studying abroad.
          </p>
        </div>

        <div className="grid items-start gap-12 lg:grid-cols-2">
          {/* Left Column: Consultant Video */}
          <motion.div
            ...
            {faqList.length > 5 && (
              <div className="mt-4 text-center lg:text-left">
                <button
                  ...
                  className="inline-flex items-center gap-2 font-semibold text-violet-600 hover:text-violet-700 transition-colors"
                >
                  {showAll ? (
                    <>
                    ...
                    <>
                      View More FAQs ({faqList.length - 5} more) <ChevronDown size={18} />
                    </>
                  )}
                </button>
              </div>
